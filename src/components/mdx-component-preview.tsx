...
import PreviewWrapper from "./mdx-preview-wrapper";
...
const Loading = () => (
  <div className="flex items-center justify-center p-4 text-fg-tertiary text-sm">
...
const ComponentNotFound = ({ name, type }: { name: string; type?: string }) => (
  <PreviewWrapper>
    <div className="flex items-center justify-center p-8 text-center">
      <div>
        <p className="mb-2 text-fg-tertiary text-sm">
          {type
            ? `${type.charAt(0).toUpperCase() + type.slice(1)} component`
            : "Component"}{" "}
          <code className="relative rounded bg-bg-tertiary px-[0.3rem] py-[0.2rem] font-medium font-mono text-sm">
            {name}
          </code>{" "}
